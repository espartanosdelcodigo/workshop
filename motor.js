...
import { megastofeles, obtenerEspejo } from "./megaestofeles.js";
...
function abrirCofre(globalGameObject) {
  console.log(
    "%c ¡COFRE! 🎁",
    "background-color: orange; color: white; font-size: 20px; padding: 10px"
  );
  let items = [
    obtenerTijera,
    obtenerEspejo,
...
function jugarNivel() {
  ...
  setTimeout(() => {
    console.log(
      "%c Entrando al nivel " + numeroNivel + "...",
  ...
  setTimeout(() => {
    console.log(
      `%c - RESULTADOS DESPUES DEL NIVEL ${numeroNivel} -`,
      "background-color: yellow; color: black; font-size: 20px; padding: 10px"
    );
  }, 5000);
